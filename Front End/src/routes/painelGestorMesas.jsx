...
export default function PainelGestorMesas() {
  ...
  const { data: mesas, isLoading: mesasLoading, error: mesasError } = useQuery({
    ...
    queryFn: async () => {
      if (cafeData) {
        const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/mesas/${cafeData.ID_Cafe}`, {
          withCredentials: true,
        });
        return response.data;
      }
  ...
  const createMesaMutation = useMutation({
    mutationFn: async (newMesa) => {
      const response = await axios.post(`${import.meta.env.VITE_API_URL}/api/mesas`, newMesa, {
        withCredentials: true,
      });
      return response.data;
    },
  ...
  const updateMesaMutation = useMutation({
    mutationFn: async ({ id, nome_mesa, lugares }) => {
      const response = await axios.patch(`${import.meta.env.VITE_API_URL}/api/mesas/${id}`, { nome_mesa, lugares }, {
        withCredentials: true,
      });
      return response.data;
    },
  ...
  const deleteMesaMutation = useMutation({
    mutationFn: async (id) => {
      await axios.delete(`${import.meta.env.VITE_API_URL}/api/mesas/${id}`, {
        withCredentials: true,
      });
    },
    onSuccess: () => {
