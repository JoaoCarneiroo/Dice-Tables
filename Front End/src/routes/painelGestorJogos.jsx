...
export default function PainelGestorJogos() {
  ...
  const { data: jogos, isLoading: jogosLoading, error: jogosError } = useQuery({
    ...
    queryFn: async () => {
      if (cafeData) {
        const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/jogos/${cafeData.ID_Cafe}`, {
          withCredentials: true,
        });
        return response.data;
      }
  ...
  const createJogoMutation = useMutation({
    mutationFn: async (novoJogo) => {
      const response = await axios.post(`${import.meta.env.VITE_API_URL}/api/jogos`, novoJogo, {
        withCredentials: true,
      });
      return response.data;
    },
  ...
  const updateJogoMutation = useMutation({
    mutationFn: async ({ id, data }) => {
      await axios.patch(`${import.meta.env.VITE_API_URL}/api/jogos/${id}`, data, {
        withCredentials: true,
      });
    },
    onSuccess: () => {
  ...
  const deleteJogoMutation = useMutation({
    mutationFn: async (id) => {
      await axios.delete(`${import.meta.env.VITE_API_URL}/api/jogos/${id}`, {
        withCredentials: true,
      });
    },
    onSuccess: () => {
